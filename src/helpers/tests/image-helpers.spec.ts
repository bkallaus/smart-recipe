...
describe('Image Extension Extraction', () => {
  ...
  test('should handle URLs with query parameters', () => {
    expect(getFileEnding('https://example.com/image.jpg?width=100')).toBe(
      'jpg',
    );
    expect(
      getFileEnding('https://example.com/image.jpg?width=100&height=100'),
    ).toBe('jpg');
  });
  ...
  test('should handle encoded query markers (%3F)', () => {
    const url =
      'recipe-images/M2sagpZH3dlTXZ94tPi9o.jpg%3Fresize%3D480%252C480%26amp%3Bssl%3D1';
