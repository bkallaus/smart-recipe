...
const slugify = (originalString:string) => {
  ...
  const convertRecipe = (recipeJson: RecipeJson, originalUrl: string) => {
    ...
    const steps = recipeJson.recipeInstructions?.sort((a, b) =>a.position  = b.position).map((instruction: any) => [instruction.name, instruction.text].filter(Boolean).join(' : '))  ;
...
export const convertJsonLdToIngest = async (jsonLd: Record<string, any>[], originalUrl:string): Promise<IngestRecipe|null> => {
