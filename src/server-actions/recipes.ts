'use server';
import { FullRecipe, Recipe } from "@/types/recipe";
import { getClient } from "./pg-client";

export const getRecentRecipes = async (limit: number = 15): Promise<Recipe[]> => {

    const client = await getClient();

    const result = await client.query('SELECT id, name, description FROM recipe order by id desc LIMIT $1', [limit]);
...
export const getFullRecipeById = async (id: number): Promise<FullRecipe> => {
    ...
    const result = await client.query('SELECT * FROM recipe WHERE id = $1', [id]);
    const ingredients = await client.query('SELECT * FROM ingredient WHERE recipe_id = $1 ORDER BY sort', [id]);
    const steps = await client.query('SELECT * FROM steps WHERE recipe_id = $1 ORDER BY sort', [id]);
...
export const insertRecipe = async (recipe: IngestRecipe) => {
    ...
    try {
        ...
        const instructionsInsert = recipe.steps.map((step, index) => {
            return client.query('INSERT INTO steps (recipe_id, label, sort) VALUES ($1, $2, $3)', [recipeId, step, index]);
        });
